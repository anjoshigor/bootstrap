...
describe('Modal', () => {
  ...
  describe('toggle', () => {
    it('should toggle a modal', done => {
      ...
      modalEl.addEventListener('shown.bs.modal', () => {
        expect(document.body.getAttribute('data-padding-right')).toEqual(
          originalPadding,
    ...
    it('should adjust the inline padding of fixed elements when opening and restore when closing', done => {
      ...
      const originalPadding = parseInt(
        window.getComputedStyle(fixedEl).paddingRight,
        10
      )
      const modalEl = fixtureEl.querySelector('.modal')
      const modal = new Modal(modalEl)
      ...
      modalEl.addEventListener('shown.bs.modal', () => {
        ...
        const currentPadding = parseInt(
          window.getComputedStyle(modalEl).paddingRight,
          10
        )

        expect(fixedEl.getAttribute('data-padding-right')).toEqual(
          '0px',
    ...
    it('should adjust the inline margin of sticky elements when opening and restore when closing', done => {
      ...
      const originalMargin = parseInt(
        window.getComputedStyle(stickyTopEl).marginRight,
        10
      )
      const modalEl = fixtureEl.querySelector('.modal')
      const modal = new Modal(modalEl)
      ...
      modalEl.addEventListener('shown.bs.modal', () => {
        ...
        expect(stickyTopEl.getAttribute('data-margin-right')).toEqual(
          '0px',
    ...
    it('should ignore values set via CSS when trying to restore body padding after closing', done => {
      ...
      styleTest.appendChild(
        document.createTextNode('body { padding-right: 7px; }')
      )
      ...
      modalEl.addEventListener('hidden.bs.modal', () => {
        expect(window.getComputedStyle(document.body).paddingLeft).toEqual(
          '0px',
    ...
    it('should ignore other inline styles when trying to restore body padding after closing', done => {
      ...
      modalEl.addEventListener('hidden.bs.modal', () => {
        ...
        expect(bodyPaddingRight === '0px' || bodyPaddingRight === '').toEqual(
          true,
          'body does not have inline padding set'
        )
  ...
  describe('show', () => {
    ...
    it('should set is transitioning if fade class is present', done => {
      fixtureEl.innerHTML =
        '<div class="modal fade"><div class="modal-dialog" /></div>'
    ...
    it('should not adjust the inline body padding when it does not overflow', done => {
      ...
      const modalEl = fixtureEl.querySelector('.modal')
      const modal = new Modal(modalEl)
      const originalPadding = window.getComputedStyle(document.body)
        .paddingRight
      ...
      modalEl.addEventListener('shown.bs.modal', () => {
        const currentPadding = window.getComputedStyle(document.body)
          .paddingRight
  ...
  describe('jQueryInterface', () => {
    ...
    it('should should not call show method', () => {
      fixtureEl.innerHTML =
        '<div class="modal" data-show="false"><div class="modal-dialog" /></div>'
